...
**Before meaningful work:** `synapse query "<topic>"` (or `synapse search`). Open
`$BRAIN_VAULT/hot.md` and top-ranked notes (`summary:` first). Reuse instead of re-deriving.

**After meaningful work:** search before create; `synapse file` / `synapse file update`;
then `synapse lint --strict` and fix until clean.
